...
// 推导函数返回值范例
int myfunc(int a, int b) // 函数类型一般是由函数的返回值和参数类型决定，与函数名没有关系，所以myfunc代表的函数类型是:
                         // int(int,int);
{
  return a + b;
}
...
decltype(std::declval<T_F>()(std::declval<U_Args>()...))
TestFnRtnImpl(T_F func, U_Args... args) {
...
// decltype(std::declval<T_F>() )：是 int (* && )(int,int),函数指针的右值引用类型，其实就简单理解成函数指针类型
