...
template <typename T> struct is_field_pointer : std::false_type {};
...
template <typename T> inline constexpr auto StructSchema() {
  return std::make_tuple();
}

#define DEFINE_STRUCT_SCHEMA(Struct, ...)                                      \
  template <> inline constexpr auto StructSchema<Struct>() {                   \
