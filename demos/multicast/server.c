...
int main(int argc, char **argv) {
  ...
  mreq.imr_interface.s_addr =
      htonl(INADDR_ANY);  // 本机的默认接口IP,本机的随机IP
  ...
  while (1) {
    ret = recvfrom(sock_fd, recv_buf, sizeof(recv_buf), 0,
    ...
    if (strstr(recv_buf, IP_FOUND)) {
      ...
      ret =
          sendto(sock_fd, send_buf, strlen(IP_FOUND_ACK) + 1, 0,
                 (struct sockaddr *)&recvaddr, socklen);  // 将数据发送给客户端
      if (ret < 0) {
        perror(" sendto! ");
      }
