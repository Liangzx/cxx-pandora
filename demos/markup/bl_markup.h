...
namespace bl
{
    ...
    {
    public:
        BlMarkup() { x_InitMarkup(); SetDoc( NULL ); };
        BlMarkup( MCD_CSTR szDoc ) { x_InitMarkup(); SetDoc( szDoc ); };
        BlMarkup( int nFlags ) { x_InitMarkup(); SetDoc( NULL ); m_nDocFlags = nFlags; };
        BlMarkup( const BlMarkup& markup ) { x_InitMarkup(); *this = markup; };
        void operator=( const BlMarkup& markup );
        ~BlMarkup();
