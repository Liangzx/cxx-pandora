...
private:
    ...
    /// <returns></returns>
    static bool CreateEVP_PKEY(unsigned char* key,
                               int is_public,
                               EVP_PKEY** out_ecKey);
...
public:
    ...
    /// <returns></returns>
    static int Verify(string in_buf,
                      const int buflen,
                      string sig,
                      const int siglen,
                      string pubkey,
    ...
    /// <returns></returns>
    static int Encrypt(string in_buf,
                       int in_buflen,
                       string& out_encrypted,
                       int& len_encrypted,
                       string pubKey);
    ...
    /// <returns></returns>
    static int Decrypt(string in_buf,
                       int in_buflen,
                       string& out_plaint,
                       int& len_plaint,
                       string prikey);
...
inline bool SM2::CreateEVP_PKEY(unsigned char* key,
                                int is_public,
                                EVP_PKEY** out_pKey)
{
    BIO* keybio = NULL;
...
inline bool SM2::PriKey2PubKey(string in_priKey, string& out_pubKey)
{
    ...
    if (ecKey == NULL) {
        std::cout << "PEM_read_bio_ECPrivateKey failed." << std::endl;
        BIO_free(keybio);
        return false;
    }
...
inline int SM2::Sign(
    string in_buf, int in_buflen, string& out_sig, int& len_sig, string priKey)
{
    int ret_val = 0;
    // 通过私钥得到EC_KEY
    EC_KEY* eckey = NULL;
    ...
    if (eckey == NULL) {
        std::cout << "PEM_read_bio_ECPrivateKey failed\n" << std::endl;
        BIO_free(keybio);
        return -2;
    }
    ...
    if (1 != ECDSA_sign(0,
                        (const unsigned char*)in_buf.c_str(),
                        in_buflen,
                        szSign,
                        (unsigned int*)&len_sig,
                        eckey)) {
        std::cout << "ECDSA_sign failed\n" << std::endl;
        ret_val = -3;
    } else {
...
inline int SM2::Verify(string in_buf,
                       const int buflen,
                       string sig,
                       const int siglen,
                       string pubkey,
                       const int keylen)
{
    ...
    if (eckey == NULL) {
        std::cout << "PEM_read_bio_EC_PUBKEY failed\n" << std::endl;
        BIO_free(keybio);
        return -2;
    }
    if (1 != ECDSA_verify(0,
...
inline int SM2::Encrypt(string in_buf,
                        int in_buflen,
                        string& out_encrypted,
                        int& len_encrypted,
                        string pubKey)
{
    ...
    if ((EVP_PKEY_set_alias_type(pkey, EVP_PKEY_SM2)) != 1) {
        std::cout << "EVP_PKEY_set_alias_type failed." << std::endl;
        goto clean_up;
    }
...
inline int SM2::Decrypt(string in_buf,
                        int in_buflen,
                        string& out_plaint,
                        int& len_plaint,
                        string prikey)
{
    ...
    if ((EVP_PKEY_decrypt_init(ectx)) != 1) {
        std::cout << "EVP_PKEY_decrypt_init failed." << std::endl;
        goto clean_up;
    }
...
int main()
{
    ...
    std::string clearText = "123456";
    std::cout << "ClearText data: " << clearText << std::endl;
    std::string cipherText;
    int len_encrypted;
    SM2::Encrypt(clearText, clearText.size(), cipherText, len_encrypted, pubKey);
    std::cout << "Encrypted data: " << byte_to_hex(cipherText) << std::endl;
    std::string plainText;
    int len_plain;
    SM2::Decrypt(cipherText, cipherText.size(), plainText, len_plain, priKey);
